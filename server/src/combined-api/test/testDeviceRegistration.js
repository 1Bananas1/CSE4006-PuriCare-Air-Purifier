...
async function testDeviceRegistration() {
  try {
    ...
    const deviceData = {
      ...
      geo: [38.6270, -90.1994], // Saint Louis University coordinates
    ...
    try {
      ...
      if (error.response) {
        ...
        if (error.response.status === 401) {
          ...
          console.log("  → Make sure your server is reading the Authorization header correctly.");
        } else if (error.response.status === 400) {
          console.log("  → The device ID might not exist in masterDeviceList collection.");
          console.log("  → Run: node test/setupTestData.js to create test device data.");
        } else if (error.response.status === 409) {
          console.log("  → This device has already been claimed by another user.");
          console.log("  → Try using a different device ID or reset the test data.");
        }
      } else {
