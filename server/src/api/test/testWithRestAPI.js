...
async function testWithRestAPI() {
  try {
    ...
    if (!email || !password) {
      ...
      console.log('Example:');
      console.log(
        '  node test/testWithRestAPI.js test@example.com MyPassword123 TEST-DEVICE-001\n'
      );
    ...
    const deviceData = {
      ...
      geo: [38.627, -90.1994], // Saint Louis University
    ...
    try {
      ...
      if (error.response) {
        ...
        if (error.response.status === 400) {
          ...
          console.log(
            "   The error is being logged in the terminal where 'npm run dev' is running."
          );
          console.log('\n🔍 Common causes:');
