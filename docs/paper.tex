...
\section{Development environment}
    ...
    \subsection{\textbf{Software in Use}}
        \begin{itemize}
            \item \textbf{ML}: PyTorch, Scipy, librosa, MLlib(optional).
            \item \textbf{Streaming/Backend}: Django(essential), Firebase, Heroku. Apache Kafka (events), Apache Spark (Structured Streaming + batch ETL); optional.
            \item \textbf{DevOps}: VS Code, Git/GitHub, Git Projects
        \end{itemize}
                    ...
                    \begin{enumerate}
                        ...
                        \item Characteristics
                            \begin{itemize}
                                ...
                                \item status (online: { type: Boolean, default: false }, lastSeen: Date)
                            \end{itemize}
                        \item Posts
                            \begin{enumerate}
                        ...
                        \item Gets
                            ...
                            \end{enumerate}
                    \end{enumerate}
                \item Firebase API/DB: Acts as the main database for user data, air quality metrics, and device information. Cannot make external API requests.
                    \textbf{Collections:}
                        \begin{itemize}
                            \item \textbf{Cities} - Stores unique city info to consolidate and cache data from regular updates
                            \item \textbf{Devices} - Links registered air purifiers to users, storing device ID, name, timezone, and geolocation coordinates
                            \item \textbf{AirQuality} - Caches air quality data including AQI, pollutant levels (CO, NO2, O3, PM2.5, PM10, SO2), location coordinates, and timestamps
                            \item \textbf{masterDeviceList} - Keeps track of all products manufactured
                            \item \textbf{timezones} - Keeps track of all active timezones so that we only query for timezones that we have products active in
                            \item \textbf{Users} - Stores user authentication (username, email, password) and profile data (phone number, respiratory services, quiet hours, device count)
                        \end{itemize}
            \end{enumerate}
