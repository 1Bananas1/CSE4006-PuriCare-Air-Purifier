...
\centering User & \centering Yim Soobeen & Identifies user needs and improves product usability. Thinks of ideas, designs functions, and suggests practical ways to implement features that address real-world problems. \\ \hline
        ...
        \caption{Task Distribution}
            \begin{center}
                ...
                \renewcommand{\arraystretch}{1.3}
                    ...
                    \centering Frontend & \centering Yim Soobeen, Kim Yeonwoo & In charge of the planning, development and rollout of the PWA onto mobile devices. \\ \hline
...
\section{Specification}
    ...
    \subsection{\textbf{Automation Routine System}}
        \begin{enumerate}
            \item Routine Data Structure (Firebase Firestore)
                \begin{itemize}
                    \item Routines use a fan-out architecture. The master copy lives under the users collection. This is where the front end creates and modifies routines. The local copy lives under the devices collection and is what the hardware reads.
                    \item Using a cloud function/trigger, it listens to the master copy of the routines and then updates the local copy upon change
                    \item Each routine has these fields: name, enabled, a schedule object containing time, days, and timezone\_offset, an action object with a type, mode, and duration, and an array of target devices. The local hardware device for each routine also keeps track of whether the routine has ran today.
                \end{itemize}
            \item Execution Logic
                \begin{enumerate}
                    \item Every second we get the current day, minute, and hour and iterate through all the routines for the device. 
                    \item If all of the conditions of a routine are met, the action is started. 
                \end{enumerate}
        \end{enumerate}
...
\section{Use Cases}
    \begin{enumerate}
        \item Login with Google Account
            \begin{table}[htbp]
                \caption{UC-1. Login with Google Account}
                    \begin{center}
                        \small
                        \setlength{\tabcolsep}{4pt} 
                        \renewcommand{\arraystretch}{1.3} 
                            \begin{tabular}{|p{4cm}|p{4cm}|}
                                \hline
                                \multicolumn{1}{c|}{\textbf{User}} & \multicolumn{1}{c|}{\textbf{System Response}} \\ \hline
                                The user launches the app and views the login screen. &  \\ \hline
                                The user selects the 'Google Account' button. & The system initiates the Google OAuth authentication process.  \\ \hline
                                The user selects their account and logs in. & The system validates the idToken and stores it in the sessionStorage. \\ \hline
                                & Upon successful login, the system navigates the user to the Home screen. \\ \hline
                                & [Alternative 3a] If login fails, the system displays a failure message. \\ \hline
                            \end{tabular}%
                        \label{tab:heroku_costs}
                    \end{center}
            \end{table}
        ...
        \item Demo Mode
            \begin{table}[htbp]
                \caption{UC-2. Use App in Demo Mode}
                    \begin{center}
                        \small
                        \setlength{\tabcolsep}{4pt} 
                        \renewcommand{\arraystretch}{1.3} 
                            \begin{tabular}{|p{4cm}|p{4cm}|}
                                \hline
                                \multicolumn{1}{c|}{\textbf{User}} & \multicolumn{1}{c|}{\textbf{System Response}} \\ \hline
                                The user selects the ‘Try the Demo’ button on the login screen. & The system activates the isDemoMode() flag to switch context to demo mode. \\ \hline
                                & The system simulates network latency using mockDelay to mimic real API behavior.  \\ \hline
                                The user views the Home screen. & The system loads MOCK-DEVICES and MOCK-ALERTS data instead of querying the live server. \\ \hline
                            \end{tabular}%
                        \label{tab:heroku_costs}
                    \end{center}
            \end{table}
            
        \item Check Air Quality \& Outdoor Weather
            \begin{table}[H]
                \caption{UC-3. Check Air Quality Summary & Outdoor Weather}
                    \begin{center}
                        \small
                        \setlength{\tabcolsep}{4pt} 
                        \renewcommand{\arraystretch}{1.3} 
                            \begin{tabular}{|p{4cm}|p{4cm}|}
                                \hline
                                \multicolumn{1}{c|}{\textbf{User}} & \multicolumn{1}{c|}{\textbf{System Response}} \\ \hline
                                The user views the Home screen. & The system fetches the device list via getDevices(). \\ \hline
                                 & The system calculates real-time AQI from PM\textsubscript{2.5} data using the simplified EPA formula and displays the status label. \\ \hline
                                The user checks the outdoor information section. & The system fetches outdoor AQI data (getOutdoorAQI) based on the device's station index. \\ \hline
                                The user selects the outdoor information section. & The system navigates to the detailed weather and map screen. \\ \hline
                            \end{tabular}%
                        \label{tab:heroku_costs}
                    \end{center}
            \end{table}
        \item Add a device
            \begin{table}[htbp]
                \caption{UC-4. Add Air Purifier Device}
                    \begin{center}
                        \small
                        \setlength{\tabcolsep}{4pt} 
                        \renewcommand{\arraystretch}{1.3} 
                            \begin{tabular}{|p{4cm}|p{4cm}|}
                                \hline
                                \multicolumn{1}{c|}{\textbf{User}} & \multicolumn{1}{c|}{\textbf{System Response}} \\ \hline
                                The user selects the ‘Add Device’ button. & The system displays options: ‘QR Code Scan’ or ‘Enter Serial Number’. \\ \hline
                                The user selects a method and enters/scans the info. & The system sends a POST request to /api/devices/register with the deviceId, name, and location. \\ \hline
                                The user completes the registration. & The system verifies the response (success: true) and registers the device to the user account. & & The system updates the device list on the Home screen. \\ \hline
                            \end{tabular}%
                        \label{tab:heroku_costs}
                    \end{center}
            \end{table}
            \newpage
            \item Check Energy Report
            \begin{table}[htbp]
                \caption{UC-5. Check Energy Usage Report}
                    \begin{center}
                        \small
                        \setlength{\tabcolsep}{4pt} 
                        \renewcommand{\arraystretch}{1.3} 
                            \begin{tabular}{|p{4cm}|p{4cm}|}
                                \hline
                                \multicolumn{1}{c|}{\textbf{User}} & \multicolumn{1}{c|}{\textbf{System Response}} \\ \hline
                                The user selects the 'Report' tab. & The system initializes the view with the default range 'Today'. \\ \hline
                                The user switches tabs (Today / This Week / This Month). & The system fetches usage, cost, and saving data corresponding to the selected range. & & The system renders a TinyBarChart component to visualize usage patterns. \\ \hline
                                The user checks the 'Energy Savings Progress' section. & The system displays a progress bar comparing the current savings rate against the monthly goal. \\ \hline
                            \end{tabular}%
                        \label{tab:heroku_costs}
                    \end{center}
            \end{table}
            \item Manage Room Structure \& Connections via Room Graph
            \begin{table}[htbp]
                \caption{UC-6. Manage Room Structure \& Connections via Room Graph}
                    \begin{center}
                        \small
                        \setlength{\tabcolsep}{4pt} 
                        \renewcommand{\arraystretch}{1.3} 
                            \begin{tabular}{|p{4cm}|p{4cm}|}
                                \hline
                                \multicolumn{1}{c|}{\textbf{User}} & \multicolumn{1}{c|}{\textbf{System Response}} \\ \hline
                                The user selects 'Room Graph' on the Home screen. & The system fetches room and connection data via getRooms and getRoomEdges APIs. & & The system renders nodes and edges using the React Flow library. \\ \hline
                                The user clicks '+ New Room' and enters a name. & The system calls the createRoom API and generates a new node at a random position. \\ \hline
                                The user drags a room node to a new position. & The system updates the node's coordinates on the screen in real-time. \\ \hline
                                The user releases the node (Drop). & The system calls the updateRoom API to persist the new (x, y) coordinates.\\ \hline
                                The user connects two room nodes. & The system calls the createRoomEdge API and renders a default 'Door' edge.\\ \hline
                                The user taps a connection edge. & The system calls updateRoomEdgeType to toggle the type to 'Airflow' and enables the flow animation.\\ \hline
                            \end{tabular}%
                        \label{tab:heroku_costs}
                    \end{center}
            \end{table}
            \newpage
        \item Manage Automation Settings
            \begin{table}[H]
                \caption{UC-7. Manage Automation Settings}
                    \begin{center}
                        \small
                        \setlength{\tabcolsep}{4pt} 
                        \renewcommand{\arraystretch}{1.3} 
                            \begin{tabular}{|p{4cm}|p{4cm}|}
                                \hline
                                \multicolumn{1}{c|}{\textbf{User}} & \multicolumn{1}{c|}{\textbf{System Response}} \\ \hline
                                The user selects ‘Automation’ from the menu. & The system loads the list of configured Routines and Schedules. \\ \hline
                                The user toggles an automation to 'Active'. & The system updates the automation state to 'Active' and sends the request to the server. \\ \hline
                                (Background) The system receives sensor data. & The system evaluates if the condition (e.g., PM\textsubscript{2.5} > 35) is met. & &
                                If triggered, the system automatically executes the defined action.\\ \hline
                            \end{tabular}%
                        \label{tab:heroku_costs}
                    \end{center}
            \end{table}
        \item Check AQI Trend Report
            \begin{table}[htbp]
                \caption{UC-8. Check AQI Trend Report}
                    \begin{center}
                        \small
                        \setlength{\tabcolsep}{4pt} 
                        \renewcommand{\arraystretch}{1.3} 
                            \begin{tabular}{|p{4cm}|p{4cm}|}
                                \hline
                                \multicolumn{1}{c|}{\textbf{User}} & \multicolumn{1}{c|}{\textbf{System Response}} \\ \hline
                                The user navigates to the AQI Trend section. & The system initiates a call to the getHistoricalSensorData API. \\ \hline
                                The user selects a time range. & The system queries historical data using startTime, endTime, and limit parameters. & & The system renders a line chart showing trends for PM\textsubscript{2.5}, CO\textsubscript{2}, and Temperature. \\ \hline
                            \end{tabular}%
                        \label{tab:heroku_costs}
                    \end{center}
            \end{table}
        \item Check and Manage User Account Information
            \begin{table}[htbp]
                \caption{UC-9. Check and Manage User Account Information}
                    \begin{center}
                        \small
                        \setlength{\tabcolsep}{4pt} 
                        \renewcommand{\arraystretch}{1.3} 
                            \begin{tabular}{|p{4cm}|p{4cm}|}
                                \hline
                                \multicolumn{1}{c|}{\textbf{User}} & \multicolumn{1}{c|}{\textbf{System Response}} \\ \hline
                                The user selects 'Account' in Settings. & The system displays the user profile information stored in the session. \\ \hline
                                The user selects Logout. & The system removes the purecare-auth token from sessionStorage. & & The system redirects the user to the login page (/login). \\ \hline
                            \end{tabular}%
                        \label{tab:heroku_costs}
                    \end{center}
            \end{table}
            \newpage
        \item Manage Privacy and Location-Based Settings
            \begin{table}[htbp]
                \caption{UC-10. Manage Privacy and Location-Based Settings}
                    \begin{center}
                        \small
                        \setlength{\tabcolsep}{4pt} 
                        \renewcommand{\arraystretch}{1.3} 
                            \begin{tabular}{|p{4cm}|p{4cm}|}
                                \hline
                                \multicolumn{1}{c|}{\textbf{User}} & \multicolumn{1}{c|}{\textbf{System Response}} \\ \hline
                                The user selects 'Location' or 'Privacy'. & The system displays the current data collection consent status. \\ \hline
                                The user modifies the settings. & The system updates the user preferences on the server.  \\ \hline
                            \end{tabular}%
                        \label{tab:heroku_costs}
                    \end{center}
            \end{table}  
        \item Device Addition Method (Detail)
            \begin{table}[htbp]
                \caption{UC-11. Device Addition Method (Detail)}
                    \begin{center}
                        \small
                        \setlength{\tabcolsep}{4pt} 
                        \renewcommand{\arraystretch}{1.3} 
                            \begin{tabular}{|p{4cm}|p{4cm}|}
                                \hline
                                \multicolumn{1}{c|}{\textbf{User}} & \multicolumn{1}{c|}{\textbf{System Response}} \\ \hline
                                The user selects 'QR Code Scan'. & The system activates the camera module for scanning.\\ \hline
                                The user scans the QR code. & The system decodes the QR code to extract the deviceId. \\ \hline
                                The user selects 'Serial Number'. & The system provides an input form for manual entry. \\ \hline
                            \end{tabular}%
                        \label{tab:heroku_costs}
                    \end{center}
            \end{table} 
        \item Manage Registered Devices
            \begin{table}[htbp]
                \caption{UC-12. Manage Registered Devices}
                    \begin{center}
                        \small
                        \setlength{\tabcolsep}{4pt} 
                        \renewcommand{\arraystretch}{1.3} 
                            \begin{tabular}{|p{4cm}|p{4cm}|}
                                \hline
                                \multicolumn{1}{c|}{\textbf{User}} & \multicolumn{1}{c|}{\textbf{System Response}} \\ \hline
                                The user selects a device from the list. & The system displays the control panel.\\ \hline
                                The user adjusts settings. & The system calls control APIs such as setFanSpeed or toggleAutoMode.\\ \hline
                                The user selects 'Delete Device'. & The system calls the deleteDevice API and removes the device from the list. \\ \hline
                            \end{tabular}%
                        \label{tab:heroku_costs}
                    \end{center}
            \end{table} 
